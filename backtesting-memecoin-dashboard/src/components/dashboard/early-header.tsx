...
import { resetEarlySniperWallet } from "@/app/actions/early-sniper-data";

export function EarlyHeader() {
    const handleReset = async () => {
        if (confirm("RESET EARLY_SNIPER_TRADES AND SOL BALANCE TO 10.0?")) {
            const res = await resetEarlySniperWallet();
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-neutral-900 pb-6 mb-2">
            <div className="space-y-1">
                ...
                <h1 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter">
                    EARLY_SNIPER_PROTOCOL <span className="text-[10px] md:text-xs font-normal text-neutral-600 font-mono tracking-widest ml-2 border border-neutral-800 px-2 py-0.5">V1.0_SNIPER</span>
                </h1>
                <p className="text-[9px] md:text-xs text-neutral-600 font-mono uppercase tracking-[0.2em]">Ultra-Low Latency PumpPortal Sniper // Isolated Infrastructure</p>
            </div>
