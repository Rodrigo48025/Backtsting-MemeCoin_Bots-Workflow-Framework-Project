...
import { getGraduationSniperBotStatus, getGraduationSniperLogs } from "@/app/actions/graduation-sniper-data";
import { Card, CardContent } from "@/components/ui/card";

export function GraduationBotVerticalStack() {
    const [expandedBot, setExpandedBot] = useState<string | null>(null);

    const fetcher = async () => {
        const [statusRes, logsRes] = await Promise.all([
            getGraduationSniperBotStatus(),
            getGraduationSniperLogs(15)
        ]);
    ...
    const { data, error } = useSWR('graduation-bot-stack', fetcher, { refreshInterval: 2000 });
