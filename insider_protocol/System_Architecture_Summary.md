# System Architecture Summary: Insider Protocol
...
2. **`insider_watcher` (The Matcher)**:
   ...
   - If a match is found, it confirms an "insider" is trading and publishes a JSON payload (containing `mint`, `insider_address`, `funding_source`) to the Redis Pub/Sub channel `insider_triggers`.

3. **`insider_sniper` (The Assassin)**:
   - Subscribes to the `insider_triggers` Redis channel.
   - Upon receiving a trigger, executes a paper trade by connecting to PostgreSQL (`insider_db`).
   - Deducts exactly 1.0 SOL from `paper_wallets` for the `INSIDER_MAIN_WAREHOUSE`.
   - Logs the position in the `insider_trades` table with an 'OPEN' status and a mock entry price of `0.00000001`.
...
- **Infrastructure**: Analysis of `docker ps` and `docker compose logs` indicates that all corresponding containers (`insider_db`, `insider_redis`, `insider_scout`, `insider_watcher`, `insider_sniper`) cleanly shut down around 16:16 UTC. 
...
1. **Trench Parameters Are Mocked / Missing**:
   - **Entry MC (Market Cap)**: `insider_watcher` receives `vSolInBondingCurve` from PumpPortal, but does not use it to filter trades. The sniper buys regardless of market cap.
...
2. **CEX-to-Sniper Hop Count Is Fixed at 1**:
   - The system is completely blind to multi-hop wallet laundering. `insider_scout` only identifies wallets that receive funds *directly* from the CEX (`keys[0] = CEX`, `keys[1] = destination`). If the insider uses an intermediary wallet, they bypass detection.
...
4. **Missing Deduplication (Sniper)**:
   - If an insider buys the same token twice within a few seconds, the sniper attempts multiple overlapping inserts and deductions.
