# Insider Protocol: System Flow (Redis Watchlist Bridge)

The `insider_protocol` leverages a high-speed Redis bridge to connect asynchronous liveness data (Scout) with real-time trade signals (Watcher).
...
graph TD
    ...
    subgraph "Watcher (The Handoff)"
        ...
        G --> H[PUBLISH insider_triggers TargetPayload]
    end
...
- **Filtering**: If the destination wallet has very few transactions or was created recently, it is marked as a "Potential Insider."
- **TTL Cache**: The wallet is stored in Redis for **30 minutes**. Insider trades typically occur very shortly after funding.

### 2. Insider Watcher
- **Throughput**: Watches every single PumpPortal trade.
- **Latency**: Performs an O(1) Redis lookup for every `trader` address.
- **State**: Does not maintain internal state of who is an insider; relies entirely on the distributed Redis cache.

### 3. Insider Sniper
- **Execution**: Receives a payload containing the `mint` and the `insider_wallet`.
